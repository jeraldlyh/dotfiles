...
const KeyColorMap key_color_map[] = {
        { BT_HST1,  { 255, 0, 0 } }, // Red
        { BT_HST2,  { 255, 0, 0 } },
        { BT_HST3,  { 255, 0, 0 } },
        { P2P4G,    { 255, 0, 0 } },
        { RGB_TOG,  { 0, 255, 0 } }, // Green
        { RGB_MOD,  { 0, 255, 0 } },
        { RGB_VAI,  { 0, 255, 0 } },
        { RGB_HUI,  { 0, 255, 0 } },
        { RGB_SAI,  { 0, 255, 0 } },
        ...
        { RGB_SAD,  { 0, 255, 0 } },
        { RGB_SPD,  { 0, 255, 0 } },
        { KC_MPRV,  { 0, 0, 255 } }, // Blue
        { KC_MPLY,  { 0, 0, 255 } },
        { KC_MNXT,  { 0, 0, 255 } },
        { KC_MUTE,  { 255, 140, 0 } }, // Amber Orange
        { KC_VOLD,  { 255, 140, 0 } },
        { KC_VOLU,  { 255, 140, 0 } },
        { KC_F1,    { 255, 140, 0 } }, 
        { KC_F2,    { 255, 140, 0 } },
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [MAC_FN1] = LAYOUT_69_ansi(
        KC_GRV,  KC_BRID,  KC_BRIU, KC_MCTRL, KC_LNPAD,_______, _______,  KC_MPRV, KC_MPLY, KC_MNXT, KC_MUTE,  KC_VOLD,  KC_VOLU,  _______,          RGB_TOG,
        _______, BT_HST1,  BT_HST2,  BT_HST3, P2P4G,   _______, RGB_MOD,  RGB_VAI, RGB_HUI, RGB_SAI, RGB_SPI,  _______,  _______,  _______,          KC_INS,
        RGB_TOG, _______,  _______,  _______, _______, _______,           RGB_RMOD,RGB_VAD, RGB_HUD, RGB_SAD,  RGB_SPD,  _______,  _______,          KC_END,
...
Color get_key_color(uint16_t keycode) {
    for (size_t i = 0; i < sizeof(key_color_map) / sizeof(key_color_map[0]); i++) {
        if (key_color_map[i].key == keycode) {
            return key_color_map[i].color;
        }
    }
    return (Color){0, 0, 0};  // Default to black (off) if no mapping found
}
...
bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
        ...
        case FN2:
            for (uint8_t row = 0; row < MATRIX_ROWS; row++) {
                for (uint8_t col = 0; col < MATRIX_COLS; col++) {
                    ...
                    int keycode = keymap_key_to_keycode(current_layer, (keypos_t){col,row});

                    if (index >= led_min && index < led_max && index != NO_LED && keycode > KC_TRNS) {
                        Color color = get_key_color(keycode);
    ...
    if (host_keyboard_led_state().caps_lock) {
        rgb_matrix_set_color_all(0x8b, 0x00, 0x00);
    }
    return false;
}
