...
UPLOAD_AND_DOWNLOAD=$(ifstat -i "$INTERFACE" -b 0.1 1 | tail -n1)
DOWNLOAD=$(echo "$UPLOAD_AND_DOWNLOAD" | awk '{print $1}' | cut -f1 -d ".")
UPLOAD=$(echo "$UPLOAD_AND_DOWNLOAD" | awk '{print $2}' | cut -f1 -d ".")

DOWNLOAD_FORMAT=$(awk -v d="$DOWNLOAD" 'BEGIN { printf(d > 999 ? "%03.0f Mbps" : "%03.0f kbps", d > 999 ? d/1000 : d) }')
UPLOAD_FORMAT=$(awk -v u="$UPLOAD" 'BEGIN { printf(u > 999 ? "%03.0f Mbps" : "%03.0f kbps", u > 999 ? u/1000 : u) }')
